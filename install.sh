...
bash -c "sudo apt-get install -y mc gcc libreadline-dev libconfig-dev libssl-dev lua5.2 liblua5.2-dev libevent-dev libjansson-dev libpython-dev make git 1> /dev/null"
bash -c "sudo apt-get install -y python3-pip 1> /dev/null"
bash -c "pip3 install pytg 1> /dev/null"
